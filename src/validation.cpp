...
static bool CheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW = true)
{
    ...
    //
    // This function only does contextless header validation (format checks).
    // The fCheckPOW parameter is kept for API compatibility but has no effect.

    // Validate nBits is a valid compact target format
    auto target = DeriveTarget(block.nBits, consensusParams.powLimit);
    if (!target) {
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "bad-diffbits", "invalid nBits format");
    }
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, const CBlockIndex* pindexPrev, bool fCheckPow = true) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // Check RandomX proof of work hash meets target
    // Skip PoW check when fCheckPow is false (e.g., for block templates)
