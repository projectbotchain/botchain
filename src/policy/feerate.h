...
const std::string CURRENCY_UNIT = "BOT"; // One formatted unit
const std::string CURRENCY_ATOM = "bots"; // One indivisible minimum value unit (botoshi)
...
/**
 * Fee rate in botoshis per virtualbyte: CAmount / vB
 * the feerate is represented internally as FeeFrac
 */
class CFeeRate
{
private:
    /** Fee rate in bots/vB (botoshis per N virtualbytes) */
    FeePerVSize m_feerate;

public:
    /** Fee rate of 0 botoshis per 0 vB */
    CFeeRate() = default;
    ...
    /**
     * Construct a fee rate from a fee in botoshis and a vsize in vB.
     *
    ...
    /**
     * Return the fee in botoshis for the given vsize in vbytes.
     * If the calculated fee would have fractional botoshis, then the
     * returned fee will always be rounded up to the nearest botoshi.
     */
    ...
    /**
     * Return the fee in botoshis for a vsize of 1000 vbytes
     */
